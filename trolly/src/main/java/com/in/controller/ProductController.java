...
public class ProductController {
	...
	public ModelAndView padd(@Valid @ModelAttribute("pdt") Product product, BindingResult result,
			@RequestParam String action, @RequestParam String name, @RequestParam("file") MultipartFile file,
		...
		case "add":
			if (!file.isEmpty()) {
				try {
					...
					return new ModelAndView("Product","message","You failed to upload " + name + " => " + e.getMessage());
				}
			} 
